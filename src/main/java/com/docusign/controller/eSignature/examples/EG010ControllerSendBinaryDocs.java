...
public class EG010ControllerSendBinaryDocs extends AbstractEsignatureController {
    ...
    private static final String PDF_DOCUMENT_FILE_NAME = "MapChimp.pdf";
    ...
    public EG010ControllerSendBinaryDocs(DSConfiguration config, Session session, User user) {
        super(config, "eg010", "Send envelope to review");
    ...
    protected Object doWork(WorkArguments args, ModelMap model, HttpServletResponse response) throws IOException {
        ...
        List<DocumentInfo> documents = List.of(
                ...
                new DocumentInfo(PDF_DOCUMENT_NAME, "2", DocumentType.PDF,
                        EnvelopeHelpers.readFile(PDF_DOCUMENT_FILE_NAME)),
                new DocumentInfo(DOCX_DOCUMENT_NAME, "3", DocumentType.DOCX,
    ...
    private static JSONObject makeEnvelopeJSON(WorkArguments args, List<DocumentInfo> documents) {
        ...
        String linkToMap = args.getLinkToMap();
        ...
        textSalary.setValue(linkToMap);
        textSalary.setLocked("true");
        textSalary.setTabId("salary");
        textSalary.setTabLabel("#HREF-ONE");
        textSalary.setTooltip(linkToMap);
        textSalary.setName(linkToMap);


        String studyID = args.getStudyID();
        Text textStudy = new Text();
        textStudy.setAnchorString("/salary/");
        textStudy.setAnchorUnits("pixels");
        textStudy.setAnchorYOffset("15");
        textStudy.setAnchorXOffset("5");
        textStudy.setFont("helvetica");
        textStudy.setFontSize("size11");
        textStudy.setBold("true");
        textStudy.setValue(studyID);
        textStudy.setLocked("true");
        textStudy.setTabId("studyID");
        textStudy.setName(studyID);

        Text reportissue = new Text();
        String linkToMap1 = "http://localhost:8080/eg016?webmapp=" + linkToMap;
        reportissue.setAnchorString("/salary/");
        reportissue.setAnchorUnits("pixels");
        reportissue.setAnchorYOffset("330");
        reportissue.setAnchorXOffset("25");
        reportissue.setFont("helvetica");
        reportissue.setFontSize("size11");
        reportissue.setBold("true");
        reportissue.setValue("report issues on the map");
        reportissue.setLocked("true");
        reportissue.setTabId("reportissue");
        reportissue.setTabLabel("#HREF-ONE");
        reportissue.setTooltip(linkToMap1);
        reportissue.setName(linkToMap1);
        ...
        textLegal.setValue("...");
        textLegal.setHeight("60");
        textLegal.setWidth("150");
        ...
        signerTabs.setTextTabs(Arrays.asList(textSalary, textLegal, textStudy, reportissue));
